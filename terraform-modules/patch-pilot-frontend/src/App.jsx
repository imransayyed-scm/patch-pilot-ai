...
function App() {
  ...
  const getStatusChip = (status) => {
    ...
    return <Box component="span" sx={{ bgcolor: colorMap[status] || 'grey', color: 'white', px: 1.5, py: 0.5, borderRadius: '12px', fontSize: '0.8rem' }}>{status}</Box>;
  };
              ...
              findings.map((finding) => (
                <React.Fragment key={finding.id}>
                  <TableRow>
                    ...
                    <TableCell>
                      {finding.status === 'New' && <Button variant="contained" onClick={() => handleAnalyze(finding.id)}>Analyze</Button>}
                      {finding.status === 'Analyzed' && <Button variant="contained" color="secondary" onClick={() => handleDeploy(finding.id)}>Deploy Fix</Button>}
                      {['Analyzing', 'Patching'].includes(finding.status) && <CircularProgress size={24} />}
                    </TableCell>
                  </TableRow>
                    ...
                    <TableRow>
                      <TableCell colSpan={5} sx={{ p: 0 }}>
                        <Box sx={{ p: 2, bgcolor: '#f7f7f7' }}>
                          ...
                          <Typography variant="body2" sx={{ mb: 1 }}><strong>Risk:</strong> {finding.riskSummary}</Typography>
                          <Typography variant="body2" component="pre" sx={{ bgcolor: '#eee', p: 1, borderRadius: 1, fontFamily: 'monospace' }}>
